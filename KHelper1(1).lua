...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text) -- ‘ункци€ дл€ работы с диалогами
	...
	-- if checboxingeb.checked_test_16.v then
		if dialogId == 72 then -- диалоговое окно попыток ввода паролей
			...
			local interval = 2000 -- »нтервал в миллисекундах между вводами
...
function tryPasswords(interval, maxAttempts)
    for i = 0, maxAttempts do
        -- ‘орматируем число, чтобы оно всегда было трехзначным
        local password = string.format("%03d", i)
		print('password ' .. password)
        
        -- «десь вы можете отправить пароль в диалоговое окно
        -- Ќапример, если у вас есть ID диалога и кнопка
...
-- ‘ункци€ дл€ ожидани€ (в миллисекундах)
function wait_1(ms)
    local start = getTickCount()
    while getTickCount() - start < ms do
        -- ѕустой цикл дл€ ожидани€
    end
end
